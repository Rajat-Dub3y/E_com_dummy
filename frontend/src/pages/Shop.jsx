...
import { setcategories,setProduct,setchecked } from "../redux/features/shop/shopSlice"
...
const Shop = () => {
  ...
  useEffect(()=>{
    if(!checked.length || !radio.length){
      if(!filteredProductQuery.isLoading){
        const filteredProducts=filteredProductQuery.data.filter(
          (product)=>{
            return(
  ...
  const uniqueBrands=[...Array.from(
    new Set(
      filteredProductQuery.data?.map((product)=>product.brand).filter((brand)=>brand!==undefined)
    )
  )]
      ...
      <div className="container mx-auto">
        <div className="flex md:flex-row">
          <div className="bg-white p-3 mt-2 mb-2">
            <h2 className="h4 text-center py-2 bg-white rounded-ful mb-2">Filter by Category</h2>
            ...
            <h2 className="h4 text-center py-2 bg-white rounded-full mb-2">Filter by Brand</h2>
            <div className="p-5">
              {uniqueBrands?.map((brand)=>(
                <div key={brand} >
                  <div className="flex items-center mr-4 mb-5">
                    ...
                    />
                    <label htmlFor="pink-radio" className="ml-2 text-sm font-medium text-black dark:text-gray-300">{brand}</label>
                  </div>
                </div>
              ))}
            </div>
